   ...
   for (const auto &connection : *senderListHandle) {
      ...
      try {
         ...
         if ( (connection.type == ConnectionKind::AutoConnection && ! receiverInSameThread) ||
              (connection.type == ConnectionKind::QueuedConnection) || receiver->isMovetoThread()) {

            // passing true indicates the data will be copied (stored on the heap)
            auto tempObj = std::make_shared<PendingSlot> (&sender, signal_Bento.clone(), receiver, slot_Bento->clone(),
                  std::make_unique<CsSignal::Internal::TeaCup_Data<SignalArgTypes...>>(true, std::forward<Ts>(Vs)... ));

            receiver->queueSlot(tempObj, ConnectionKind::QueuedConnection);
            ...
            receiver->queueSlot(tempObj, ConnectionKind::BlockingQueuedConnection);
